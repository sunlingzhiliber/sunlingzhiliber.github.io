...
class Scene {
    ...
    clear() {
        this.ctx.fillStyle = "rgba(255, 247, 240,0.3)", this.ctx.fillRect(0, 0, this.width, this.height)
    }
    build(t) {
    ...
    nextAction() {
        this.actionIndex++, this.actionIndex >= this.actions.length && (this.actionIndex = 0), this.activeGeometry = this.geometrys[this.actionIndex], this.tick = 0, this.setParticle()
    }
    setParticle() {
    ...
    renderParticles() {
        this.particles.forEach(t => {
            const i = t.getAxis2D();
            if (this.ctx.beginPath(), "1" === getRequestParam().img) this.ctx.drawImage(HeartCanvas, i.x - 15, i.y - 15, 30, 30);
            else {
    ...
    draw() {
        this.tick++, this.tick >= this.actions[this.actionIndex].lifeTime && this.nextAction(), this.clear(), this.renderParticles(), this.raf = requestAnimationFrame(this.draw)
    }
}
