...
而且对于那茫茫何其多的、何其复杂的地理模型数据，我们能做到的就是，`写代码`
...
在参与式建模任务中，必须从模型执行端和数据资源访问端考虑数据配置工作。数据服务控制模块旨在访问数据资源，在数据服务控制模块中，每个相关的数据资源都被分配了一个数据存根 Data Stub(Link configuration)；数据存根用于帮助参与建模者处理从模型服务控制模块路由的数据要求。
